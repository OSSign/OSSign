...
const psFile = pjoin(__dirname, 'example-ts.ps1');
const psFile2 = pjoin(__dirname, 'example-ts-2.ps1');
const configFile = pjoin(__dirname, 'ossign-config-ts.json');

async function runExamples() {
  console.log('=== OSSign-JS JavaScript Examples ===\\n');
  
  // Create dummy powershell files
  await writeFile(psFile, 'Write-Host "Hello, OSSign!"');
  await writeFile(psFile2, 'Write-Host "Hello, OSSign 2!"');
