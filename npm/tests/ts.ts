// TypeScript Example
import { Sign, SignSync, GetSignerFunction } from '../dist/index';
...
async function runExamples() {
  ...
  const contentAsync = await readFile(asyncFile, 'utf8');
  // console.log('\\nSigned PowerShell file content:\\n', contentAsync);

  if (contentAsync.includes('SIG # Begin signature block')) {
    console.log('✓ PowerShell file signed successfully (Async)');
  } else {
    console.error('✗ PowerShell file signing failed (Async)');
    return;
  }
  ...
  if (contentGetFunc.includes('SIG # Begin signature block')) {
    console.log('✓ PowerShell file signed successfully (GetSignerFunctionSync)');
  } else {
    console.error('✗ PowerShell file signing failed (GetSignerFunctionSync)');
    return;
  }
